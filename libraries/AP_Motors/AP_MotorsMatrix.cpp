...
void AP_MotorsMatrix::output_to_motors()
{
    ...
    switch (_multicopter_flags.spool_mode) {
        case SHUT_DOWN:
            ...
            for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    motor_out[i] = 1500; // _throttle_radio_min;
                }
            }
            break;
        case SPIN_WHEN_ARMED:
            ...
            for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    motor_out[i] = 1500; // constrain_int16(_throttle_radio_min + _throttle_low_end_pct * _min_throttle, _throttle_radio_min, _throttle_radio_min + _min_throttle);
                }
            }
            break;
        case SPOOL_UP:
